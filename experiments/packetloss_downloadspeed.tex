Figure~\ref{ref:PacketLossDownload} below shows the download speed measured in Mbps against the increasing rate of packet loss, as you can see the download rate only reaches around 25\%, this is due to the connection test failing at anything above. The tests were performed on the localhost as to reduce any degradation that may exists already in an existing network.
...
As you can see around the 1 second mark in Figure~\ref{ref:PacketLossCwndExperiment} there are almost identical drops in the Cwnd size, this demonstrates that packet loss causes the congestion window to reduce in size and if packet loss is substantial enough and frequent enough it can cause a huge drop in transfer rates.
...
Please also note the initial increase in transfer rate in Figure~\ref{ref:PacketLossCwndExperiment}, this is displayed in Figure~\ref{ref:SlowStart} below:
