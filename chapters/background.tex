...
When there are situations where multiple clients are connected to a single receiver, the bandwidth needs to be split equally between the clients, this is performed by the inbuilt TCP congestion control. The clients send data and keep increasing their transfer rate until they begin loosing packets. Packet loss therefore is assumed to be a sign of congestion - this is because it thinks the packets are being lost because the router is dropping them due to insufficient packet buffering space. So, when the number of clients drops the more bandwidth is freed up, the congestion algorithm utilises this because it has been periodically probing the network to check for available bandwidth, it does this by increasing it transfer rate until packet loss is encountered, where it then slows quickly reduces its transfer rate and repeats the process. This results in TCP auto balancing when new clients are introduces and removed and thus utilizing as much bandwidth as is available.

The rate of transfer is defined by two values "Congestion Window" (Cwnd) and "Advertised Window". Congestion window is the amount of packets a connection stream allows in flight and the advertised window is used to inform the sending end how much buffer space is available to hold incoming packets. These both define the overall "winsize" where a small value means less packets can be sent without waiting for corresponding acknowledgements and therefore reducing the transfer speed. The equation below is used to define window size: 
...
\subsubsection*{AIMD}
AIMD standing for Additive Increase Multiplicative Decline is a feedback control algorithm used in TCP congestion control. It works by having a linear increase and exponential decline and it will naturally mean that if set up correctly all entities using AIMD will converge on equal usage of a resource, in this case network bandwidth. This famously produces a 'saw-tooth' pattern when the maximum capacity is reached.
...
There are various algorithms used to manage congestion on a network and they work is slightly different and have situations where some may perform better than others. 

\subsubsection*{TCP Reno}
TCP Reno was first released in the 4.3BSD OS and is normally grouped up with TCP Tahoe as they share many similarities. TCP Reno performs its additive increase with RTT based calculations. It also introduces two new ideas "Fast Retransmit" and "Fast Recovery":

{\bf Fast Retransmit}\\
This mechanism uses 3 duplicated acknowledgements (These can be caused by re-orderings or missing segments) to signal the need for a retransmission, this process can repair single segment loss and does not involve the use of any time-outs and therefore can involve a lot less idle link time. After this the congestion window size is reduced by half because of detection of loss.

{\bf Fast Recovery}\\
For every duplicate ACKs a segment must arrive, this can therefore be used to assume that the receiving of a segment means the buffer in the receiving end has more space and also means the sending of more packets has a high chance not to cause the buffer on the receiving end to overflow so therefore, new segments are put onto the network when acknowledgement are received (Even duplicated). These techniques are both used together and can result in effective mitigation of packet loss.
...
TCP Cubic is an algorithm designed for high bandwidth networks with {\bf high latency}. TCP Reno as explained previously relies on acknowledgements to increase the Cwnd value, this is indirectly depended on round trip time (Latency) meaning high latency networks slowly increase the Cwnd value in algorithms that additivity increase in this RTT based fashion. TCP Cubic additive increase is a cubic function of the {\bf time} since the last congestion event, this means the latency and size of a network does not affect the gradual increase of the networks transfer speed.
