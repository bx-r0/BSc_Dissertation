...
To create the access point, software called Hostapd will be used, this software allows for the configuration of access points to occur with their SSID, password, channel and encryption type to all be set up manually.

\subsubsection{DHCP}
The protocol used to assign IP address in a network is called DHCP (Dynamic Host Configuration Protocol) and will need to be set up to assign IP addresses to any connecting entity. This protocol also handles the assigning of important addresses like the default gateway and the DNS address. DHCP has two sections; server and client. The server will be hosted on the router in this case but in much larger companies and businesses the DHCP server would be hosted on its own dedicated machine. Each computer that wants to connect to a network runs a DHCP client that sends out a DHCP-Discover request as a broadcast that looks for a DHCP server on the network. All other hosts ignore the request apart from the server that will then return a DHCP-Offer. This client then again accepts the DHCP offer with a DHCP-Request that finally the DHCP server responds with a DHCP-Pack that contains the subnet, default gateway and the assigned IP address along with a period of time the IP has been leased for. This whole process allows for unique allocation of IP addresses
