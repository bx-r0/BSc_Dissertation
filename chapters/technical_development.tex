...
The two NFQUEUE rules were chosen due to their coverage; They cover all incoming packets to the router (INPUT) and all packets being routed and coming directly from the router. These rule also don't cause the same packet to be pushed into the queue more than once.
...
The types of connections were simulated using common values for latency and packet loss. 3G and 4G values were obtained from Ofcom \footnote{\url{https://www.ofcom.org.uk/about-ofcom/latest/media/media-releases/2014/3g-4g-bb-speeds}} and good and bad WiFi connections from online sources\footnote{\url{https://www.ispreview.co.uk/index.php/2017/04/ofcom-2017-study-average-uk-home-broadband-speeds-rise-36-2mbps.html}}. 
...
Figure~\ref{ref:UdpFloodingFigure} above is a section of code from the UDP flooding attack script. As mentioned previously around 10 threads will run the code at the same time in an attempt to maximise the throughput of UDP packets. The effects of the attack are visible on the targets end and causes a visual degradation in the connection speed.
...
%TODO - Explain aspects of scripts
...
The effect of the UDP flooding required experimentation. A single target was selected on a network just containing the attacker and victim and like previous tests SpeedTest.Net was used to obtain bandwidth results. Each test was run with the same computers on the same network. As you can see from the two images the effect of the UDP flooding is devastating on a connections speed, taking a fast internet connection and reverting it to a crawl. If more computer running the script were active the internet connection could be easily fully blocked.